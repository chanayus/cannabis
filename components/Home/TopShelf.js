  ...
  return (
    <section className="relative  mt-20">
      <div className="container-no-padding grid items-center lg:grid-cols-2 relative z-10 ">
        <div className="p-8 lg:px-10 bg-green relative ">
          ...
          <button className="px-8 py-3 bg-orange my-6 text-white">Shop Now</button>
          <div className="w-[2px] h-[75%] bg-white ml-2 absolute lg:block hidden top-[46%] translate-y-[-50%] left-[0] opacity-50"></div>
        ...
        <div className="p-8 relative w-full h-full ">
          ...
          <div className="bg-white dark:bg-black p-4 rounded-lg flex absolute md:w-[70%] w-[90%] left-1/2 translate-x-[-50%] md:bottom-8 bottom-0 z-10">
          ...
          <AnimatePresence>
            {!isUpdate && (
              <motion.div className="mx-auto w-[13rem] h-[22rem] " initial={{ opacity: 0, scale: 0.8, rotate: 30 }} animate={{ opacity: 1, scale: 1, rotate: 0 }} exit={{ opacity: 0, scale: 0.8 }}>
                <img src={`/images/products/${slideData[slideIndex - 1].image}`} alt="product-image" className="w-full h-full object-contain" />
              </motion.div>
            )}
          </AnimatePresence>
        </div>
